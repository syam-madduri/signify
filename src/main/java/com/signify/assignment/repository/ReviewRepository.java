...
public interface ReviewRepository extends JpaRepository<Review, Date> {
    ...
    @Query("select new com.signify.assignment.dto.ReviewDTO (to_char(reviewed_date, 'dd-mm-yyyy hh:mm:ss') , review, author, review_source, rating, title, product_name) " +
            "from Review where (rating = :rating or 0 = :rating) and (to_char(reviewed_date , 'dd-mm-yyyy hh:mm:ss') = :reviewed_date or :reviewed_date is null) " +
